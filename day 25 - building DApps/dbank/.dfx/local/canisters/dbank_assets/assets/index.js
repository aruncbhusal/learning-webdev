...
window.dbank = _declarations_dbank__WEBPACK_IMPORTED_MODULE_0__.dbank;
// Now we can add an event listener to update the current balance
window.addEventListener("load", async () => {
  const balance = await _declarations_dbank__WEBPACK_IMPORTED_MODULE_0__.dbank.checkBalance();
...
document.querySelector('form').addEventListener('submit', async (event) => {
  ...
  if (event.target.querySelector('#input-amount').value.trim().length) {
    ...
    await _declarations_dbank__WEBPACK_IMPORTED_MODULE_0__.dbank.topUp(topupAmt);
  ...
  if (event.target.querySelector('#withdrawal-amount').value.trim().length) {
    ...
    await _declarations_dbank__WEBPACK_IMPORTED_MODULE_0__.dbank.withdraw(withdrawAmt);
  }
  ...
  const currentBalance = await _declarations_dbank__WEBPACK_IMPORTED_MODULE_0__.dbank.checkBalance();
